...
####################################################################
    ...
    def optimize_parameters(self, step):
        ...
        out, maskcolor = self.netG(input=self.var_L, mask=self.mask, gt=self.real_H.detach(),rev = False)
        if self.opt['train']['dual']:
            out_rev,_ = self.netG(input=self.var_L, mask=self.mask, gt=self.real_H.detach(),rev = True)
